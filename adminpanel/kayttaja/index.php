...
<body>
<?php
//Jos käyttäjä on admin, includataan adminin navi ja käyttäjäsivu sekä asetetaan käyttäjänappi aktiiviseksi.
...
if (isset($_SESSION['kayttaja']) && !empty($_SESSION['kayttaja'])) {
	if ($_SESSION['kayttaja'] == "admin") {
		...
		?>
		<script>$('li.kayttaja').addClass("active");</script>
		<?php
	}
}
//Muussa tapauksessa tulostetaan kirjautumissivulle ohjaava nappi.
else {
	echo '<div class="text-center"><h2>Et ole kirjautuneena järjestelmänvalvojana.</h2><button class="btn btn-success" onclick="window.location.href=\'../../admin/\'">Tästä kirjautumissivulle.</button></div>';
}
?>
</body>
</html>